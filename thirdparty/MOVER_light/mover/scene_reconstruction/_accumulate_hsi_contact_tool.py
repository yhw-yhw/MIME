...
def body2objs(self, body_v, body_vn, objs_v_list, name=None,\
    ...
    for i in range(len(self.idx_each_object)):
        ...
        mean_body = body_v.mean(0)

        distance = (obj - mean_body).norm(dim=2).min().item()
        if distance < 1.0:
            dis_list.append(distance)
        else:
            dis_list.append(100)
    print('dis_list: ', dis_list)
    nearest_obj_idx = np.argmin(np.array(dis_list))
...
def load_contact_body_to_objs(self, ply_file_list, contact_file_list, ftov, contact_parts='body', debug=True, output_folder=None):
    ...
    if os.path.exists(os.path.join(output_folder, f'accumulate_contact_{contact_parts}_vertices.npy')):
...
def assign_contact_body_to_objs(self, ply_file_list, contact_file_list, ftov, contact_parts='body', \
    ...
    reload_contact = False
    logger.info(f'reload contact information; {reload_contact}')
    ...
    if reload_contact: # only for contact feet vertices.
                ...
                if contact_parts in ['body', 'handArm'] and assign2obj:
                    ...
                    if ADD_HAND_CONTACT:
                        ...
                        assign_obj = self.body2objs(v[contact_labels[:, 0]], body_v_normals[contact_labels[:, 0]],\
                            verts_parallel_ground_list, name=ply_file, contact_parts=contact_parts)
        ...
        if debug and torch.cat(accumulate_contact_v).shape[0] != 0:
            ...
            logger.info(f'save to {template_save_fn}')
            self.viz_verts(tmp_cv, template_save_fn, verts_n=tmp_cvn)
            ...
            if contact_parts in ['body', 'handArm'] :
                ...
                for n_i, assign_idx in enumerate(assign_obj_list):
                    ...
                    if assign_idx != -1:
                        color = np.clip(np.array(define_color_map[4]) + assign_idx * 0.03, 0.01, 0.99)
                    else:
                ...
                self.viz_verts(tmp_cv, template_save_fn, verts_c=vert_colors)
            ...
            self.viz_verts(all_contact_v, template_save_fn, verts_n=all_contact_vn)
...
# this works for feet contact vertices.
def voxelize_contact_vertices(self, ori_contact_v, ori_contact_vn, voxel_size, grid_min, dim, \
    ...
    start = time.time()
    v_volume = torch.zeros(dim).to(device)
    cvn_volume = torch.zeros((dim[0],dim[1],dim[2], 3)).to(device)
    cb2j_volume = torch.zeros(dim).to(device).long()
    
    query_points = torch.floor((ori_contact_v-grid_min) / voxel_size).long().squeeze()
    ...
    for i in tqdm(range(query_points.shape[0]), desc='voxelize contact vertices'):
        ...
        cvn_volume[tx, ty, tz] = (cvn_volume[tx, ty, tz] * v_volume[tx, ty, tz] + contact_vn[i]) / (v_volume[tx, ty, tz]+1)  
        v_volume[tx,ty,tz] += 1
    ...
    if debug and save_dir is not None and contact_v.shape[0] > 0:
        ...
        self.viz_verts(contact_v.cpu().numpy(), tmp_save_path, verts_n=contact_vn.cpu().numpy())
