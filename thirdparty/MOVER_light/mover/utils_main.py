...
from sub_thirdparty.body_models.smplifyx.utils_mics.utils import get_image
...
def save_mesh_models(model, save_dir):
    ...
    # _, gp_model_world = model.get_checkerboard_ground_np()
    # gp_model_world.export(os.path.join(save_dir, f'gp_mesh_world.obj'))

    print('save mesh ok.')

def save_scene_model(model, save_dir, tmp_name):
    
    # output mesh in camera CS
    print(f'save to {os.path.join(save_dir, tmp_name)}')
